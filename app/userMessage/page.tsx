...
export default function Messages() {
  ...
  // Fetch user data, validate session, and fetch all users including admin
  useEffect(() => {
  ...
  // Fetch all conversations where current user is sender or receiver
  useEffect(() => {
    const fetchConversations = async () => {
      if (!userEmail || users.length === 0) {
        console.log('No user email or users, skipping conversation fetch');
        return;
      }
      ...
      try {
        ...
        for (const doc of messagesSnapshot.docs) {
          ...
          const userAsSender = messages.some(msg => msg.senderEmail === userEmail);
          const userAsReceiver = messages.some(msg => msg.receiver === userEmail);
          
          if (!userAsSender && !userAsReceiver) continue;

          let otherUserEmail = '';

          if (userAsSender) {
            otherUserEmail = messages.find(msg => msg.senderEmail === userEmail)?.receiver || '';
          } else if (userAsReceiver) {
            otherUserEmail = messages.find(msg => msg.receiver === userEmail)?.senderEmail || '';
          }
          ...
          if (otherUserEmail && !userEmails.has(otherUserEmail)) {
            ...
            const unreadCount = messages.filter(msg => msg.receiver === userEmail && !msg.read).length;
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      ...
      try {
        ...
        if (!conversation?.docId) {
          setMessages([]);
          console.log(`No conversation found for ${selectedUser.email}`);
          return;
        }

        const messageDocRef = doc(db, 'messages', conversation.docId);
        ...
        if (messageSnap.exists()) {
          ...
          fetchedMessages = (messageData.messages || []).map((msg: any) => {
            ...
            const receiver = users.find(u => u.email === msg.receiver) || {
              email: msg.receiver || '',
              firstName: msg.receiver === 'admin' ? 'Admin' : 'User',
            };
            return {
              content: msg.content || '',
    ...
    fetchMessages();
  }, [selectedUser, userEmail, users, t, conversations]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const conversationId = getConversationId(userEmail, selectedUser.email);
